...
class MBTilesCache(TileCacheBase):
    ...
    def db(self):
        if not getattr(self._db_conn_cache, 'db', None):
            self.ensure_mbtile()
            self._db_conn_cache.db = sqlite3.connect(f"file:{self.mbtile_file}?mode=ro", self.timeout)
...
class MBTilesDimensionsCache(TileCacheBase):
    ...
    def __init__(self, mbtiles_dir, dimensionlist, timeout=30, wal=False, dim_separator="\034", ttl=0, coverage=None):
        super(MBTilesDimensionsCache, self).__init__(coverage)
        md5 = hashlib.new('md5', mbtiles_dir.encode('utf-8'), usedforsecurity=False)
        self.lock_cache_id = 'sqlite-' + md5.hexdigest()
